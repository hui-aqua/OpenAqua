...
class irregular_sea:
    ...
    *NOTE: The maximum applied simulation time should be less than 3h.
    ...
    def __init__(self, significant_wave_height, peak_period, gamma, water_depth, wave_direction):
        ...
        self.list_of_waves=[]
        time_max=3600  # 3h, We assume the simulations will not exceed 3h.
...
if __name__ == "__main__":
    ...
    k=sea_state.get_velocity_with_time(np.array([0,0,0]),time_frame)
    print(k.shape)
    print(max(np.linalg.norm(k,axis=1)))
    plt.plot(time_frame, np.linalg.norm(k,axis=1))

    plt.plot(time_frame, np.linalg.norm(sea_state.get_velocity_with_time(np.array([0,0,0.1]),time_frame),axis=1))
    plt.xlabel("time (s)")
